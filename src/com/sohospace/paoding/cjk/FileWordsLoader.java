...
public class FileWordsLoader implements WordsLoader {
	...
	private String path = "";// FileWordsLoader.class.getClassLoader().getResource("").getPath()
								// + "/META-INF/dic/CJK";
	...
	public LinkedList<String> loadCJKVocabulary() {
		...
		LinkedList<String> base = ejk.get(baseName);
		 ...
		 Merger.merge(divAsianChina, loadDirectory(path + "/AD"));
		 Merger.merge(base, divAsianChina);
		 Merger.remove(base, ejk.get("xDisturbWords"));
		return base;
	}
	...
	public LinkedList<String> loadCJKConfucianFamilyNames() {
		try {
			return FileWordsReader.readWords(path + "/xSurnames.dic").values()
					.iterator().next();
		} catch (IOException e) {
			throw toRuntimeException(e);
		}
	...
	public LinkedList<String> loadCJKXwords() {
		try {
			return FileWordsReader.readWords(path + "/xDisturbWords.dic")
	...
	public LinkedList<String> loadCJKXchars() {
		try {
			return FileWordsReader.readWords(path + "/xDisturbChars.dic")
					.values().iterator().next();
		} catch (IOException e) {
			throw toRuntimeException(e);
		}
	...
	public LinkedList<String> loadCJKUnit() {
		try {
			return FileWordsReader.readWords(path + "/xMeterageUnits.dic")
					.values().iterator().next();
		} catch (IOException e) {
			throw toRuntimeException(e);
		}
